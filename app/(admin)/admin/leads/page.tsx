import { Phone, Mail, MessageSquare, Calculator, Shield } from "lucide-react";
...
export default async function AdminLeadsPage({ searchParams }: PageProps) {
  ...
  const where: { status?: LeadStatus; type?: LeadType } = {};
    ...
    <div>
      ...
      {leads.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center">
          <p className="text-muted-foreground font-medium">לא נמצאו לידים</p>
          <p className="text-sm text-muted-foreground/70 mt-1">
            נסה לשנות את הסינון או להסיר אותו
          </p>
        </div>
      ) : (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <span className="text-sm text-muted-foreground">{lead.dealer.businessName}</span>
