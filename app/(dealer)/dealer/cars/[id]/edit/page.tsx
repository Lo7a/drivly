...
export default async function DealerEditCarPage({ params }: PageProps) {
  ...
  if (!car || car.dealerId !== user.dealer.id) {
    notFound();
  }
  ...
  return (
    <CarForm
      ...
      heading={`עריכת רכב: ${car.make} ${car.model} ${car.year}`}
      redirectTo="/dealer/cars"
    />
  );
}
